...
func (s *DockerNetworkSuite) TestDockerNetworkUnsupportedPreferredIP(c *check.C) {
	...
	dockerCmd(c, "run", "-d", "--name", "c0", "busybox", "top")
	for _, mode := range []string{"none", "host", "bridge", "container:c0"} {
...
func checkUnsupportedNetworkAndIP(c *check.C, nnwMode string) {
	out, _, err := dockerCmdWithError("run", "-d", "--ip", "172.28.99.88", "--ip6", "2001:db8:1234::9988", "busybox", "top")
	c.Assert(err, checker.NotNil, check.Commentf("out: %s", out))
